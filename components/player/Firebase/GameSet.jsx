...
import { updateHitPeriodically } from "@/components/fetcher/FireBaseNameLogic";
...
export default function GameSet() {
  ...
  const { startUpdating, stopUpdating } = updateHitPeriodically(
    database,
    name,
    () => countRef.current
  );
  ...
  useEffect(() => {
    ...
    // Update Redux every 5 seconds
    const reduxUpdateInterval = setInterval(() => {
      dispatch(playerDataActions.updateScore(countRef.current));
    }, 5000);

    return () => {
      stopUpdating();
      clearInterval(reduxUpdateInterval);
    };
  }, [name]);
  ...
  const animate = (timestamp) => {
    if (shouldAnimate && headingRef.current) {
  ...
  return (
    <>
      ...
      <div className="flex flex-col w-full h-full overflow-hidden relative select-none">
        <h1
          ref={headingRef}
          className="w-full text-center py-4 border-b-2 border-[#002060] text-[36px] font-[600] text-[#002060]"
        >
          目前分數 {count}
        </h1>
        <div
